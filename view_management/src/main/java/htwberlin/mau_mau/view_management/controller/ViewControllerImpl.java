    ...
    boolean processMove(GameData gameData, Player player) {
        ...
        if (player instanceof RealPlayer) {
            ...
            if (move >= 1 && move <= player.getHand().getCards().size()) {
                boolean success = gameService.makeGameMoveForRealPlayer(move - 1, gameData);
                if (success) {
                    LOGGER.debug("*** YOU PLAYED " + gameData.getOpenCard().getRank().toString() + " of "
                            + gameData.getOpenCard().getSuit().toString());
                    ui.showPlayedCard(success, "You", gameData.getOpenCard(), oldOpenCard);
                    ...
                    ui.showPlayedCard(false, "You", gameData.getOpenCard(), oldOpenCard);
                }
            ...
            boolean success = gameService.makeGameMoveForVirtualPlayer(gameData.getOpenCard(), player.getHand(), gameData);
            ...
            if (success) {
                ...
                ui.showPlayedCard(success, player.getName(), gameData.getOpenCard(), oldOpenCard);
            } else {
                try {
                    Card drawnCard = cardService.drawCard(gameData.getDrawingStack(), gameData.getPlayingStack(),
                            player.getHand());
